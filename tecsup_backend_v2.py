...
Versión mejorada con retry automático y mejor manejo de errores
"""
...
def retry_on_error(func, max_retries=MAX_RETRIES, delay=RETRY_DELAY):
    """
    Decorator para reintentar automáticamente en caso de error 503
    """
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Si es error 503 (sobrecarga), reintentar
...
def transcribe_with_gemini(audio_path, progress_callback=None):
    """Transcribe audio using Gemini API con retry automático"""
    try:
        ...
        # Usar modelo estable (gemini-1.5-flash en lugar de 2.0-flash-exp)
...
def generate_summary_and_script(transcription, progress_callback=None):
    """Generate summary and podcast script using Gemini con retry"""
    try:
        if progress_callback:
...
def process_audio():
    """Main endpoint to process audio file - con retry automático"""
    try:
        data = request.json
        ...
        # Paso 1: Transcribir audio (con retry automático)
        print("\n=== PASO 1: TRANSCRIPCIÓN (con retry automático) ===")
        ...
        # Paso 2: Generar resumen y guion (con retry automático)
        print("\n=== PASO 2: GENERACIÓN DE CONTENIDO (con retry automático) ===")
        ...
        return jsonify({
            ...
            'metadata': {
                ...
                'retries_used': 'auto-retry habilitado'
            },
        ...
        if '503' in error_msg or 'overloaded' in error_msg.lower():
            user_message = "El servidor de Gemini AI está temporalmente sobrecargado. Por favor, espera 1-2 minutos e intenta nuevamente."
        elif '429' in error_msg:
            user_message = "Has excedido el límite de peticiones. Espera un momento e intenta de nuevo."
        elif 'API key' in error_msg:
            user_message = "Error de autenticación con Gemini AI. Verifica la API key."
        else:
            user_message = f"Error al procesar: {error_msg}"
...
if __name__ == '__main__':
    ...
    print("  ✅ Retry automático en errores 503 (servidor sobrecargado)")
    print("  ✅ Exponential backoff (espera inteligente entre reintentos)")
    print("  ✅ Modelo estable: gemini-1.5-flash")
    print(f"  ✅ Máximo {MAX_RETRIES} reintentos automáticos")
